...
def model(LR_input, HR_output, scale, (d, s, m)):
    """
    ...
    filters = [
        ...
        tf.Variable(tf.random_normal([9, 9, s, 1], stddev=0.1), name="f%d" % (4 + m)) # (f5,n5,c5) = (9,1,12)
    ]
    bias = [
    ...
    x = tf.nn.conv2d_transpose(x, filters[4+(m-1)], [1, 1, 1, 1], padding='SAME', name="deconv")
    out = x + bias[4+(m-1)]
